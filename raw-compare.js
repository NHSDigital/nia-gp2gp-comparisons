	...
	for (var resourceTypeIndex = 0; resourceTypeIndex < entriesByResource.resourceTypes.length; resourceTypeIndex++) { //entriesByResource.resourceTypes.length
		...
		switch (resourceType) {
			...
			case "DiagnosticReport": {
				outputReport = outputReport + resourceTypeCompareBuilder(resourceType, resourceTypeIndex, diagnosticReportMatchFields, entriesByResource.emisEntries, entriesByResource.psEntries, allPsEntires)
				break;
			}
		}
	}
	...
	for (var emisEntryCount = 0; emisEntryCount < emisEntries[resourceTypeIndex].length; emisEntryCount++) {
		...
		while (
			...
			for (var psEntryCount = 0; psEntryCount < searchEntries.length; psEntryCount++) {
				...
				if (emisEntryMatchPropertyValue !== null && psEntryMatchPropertyValue !== null) {
					
					

					// TPP edge case
					if ((matchFields[matchingFieldIndex] === "type.[0].text" || matchFields[matchingFieldIndex] === "title" || matchFields[matchingFieldIndex] === "date") && (emisEntryMatchPropertyValue.indexOf(psEntryMatchPropertyValue) !== -1) ) {
						matchingEntries.push(psEntry);
						console.log("start - contains")
						console.log(matchFields[matchingFieldIndex])
						console.log(emisEntryMatchPropertyValue)
						console.log(psEntryMatchPropertyValue)
					} 
		...
		if (searchEntries.length == 1) {
			...
			outputReport = outputReport + "EMIS " + resourceName + " Entry " + (emisEntryCount + 1) + "\n";
			outputReport = outputReport + "MISSING Entry: EMIS entry with ID " + emisEntryId + " has not been found in the PS entries \n"
...
function scanForIdsAndReplace(emisOutputFileJson, psOutputFileJson) {
	...
	for (var idReplaceIndex = 0; idReplaceIndex < idsToReplace.length; idReplaceIndex++) {
		...
		if (typeof(idsToReplace[idReplaceIndex].emisEntryId) !== 'undefined' )
	...
	for (var emisEntryCount = 0; emisEntryCount < emisEntries[resourceTypeIndex].length; emisEntryCount++) {
		...
		while (
			...
			for (var psEntryCount = 0; psEntryCount < searchEntries.length; psEntryCount++) {
				...
				//if we've found a match, log it.
				if (emisEntryMatchPropertyValue !== null && psEntryMatchPropertyValue !== null) {
					if (emisEntryMatchPropertyValue === psEntryMatchPropertyValue || emisEntryMatchPropertyValue.indexOf(psEntryMatchPropertyValue) !== -1) {
						matchingEntries.push(psEntry);
					}
				}
			}
